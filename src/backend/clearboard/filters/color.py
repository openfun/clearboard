"""image proccessing transforming image to enhance a whiteboard"""
import cv2
import numpy as np


def normalize_kernel(kernel, k_width, k_height, scaling_factor=1.0):
    """Zero-summing normalize kernel"""

    k_eps = 1.0e-12
    # positive and negative sum of kernel values
    pos_sum, neg_sum = 0, 0
    for i in range(k_width * k_height):
        if abs(kernel[i]) < k_eps:
            kernel[i] = 0.0
        if kernel[i] < 0:
            neg_sum += kernel[i]
        else:
            pos_sum += kernel[i]

    if abs(pos_sum) < k_eps:
        pos_sum = 1.0
    if abs(neg_sum) < k_eps:
        neg_factor = 1.0

    pos_factor = scaling_factor / pos_sum
    neg_factor = scaling_factor / neg_sum

    # scale kernel values for zero-summing kernel
    for i in range(k_width * k_height):
        if not np.nan == kernel[i]:
            kernel[i] *= pos_factor if kernel[i] >= 0 else neg_factor

    return kernel


def dog(img, k_size, sigma_1, sigma_2):
    """Difference of Gaussian by subtracting kernel 1 and kernel 2"""

    x_coord = y_coord = (k_size - 1) // 2
    kernel = np.zeros(k_size * k_size)

    # first gaussian kernal
    if sigma_1 > 0:
        coeff_1 = 1 / (2 * sigma_1 * sigma_1)
        coeff_2 = 1 / (2 * np.pi * sigma_1 * sigma_1)
        i = 0
        for current_y_coord in range(-y_coord, y_coord + 1):
            for current_x_coord in range(-x_coord, x_coord + 1):
                kernel[i] = (
                    np.exp(
                        -(
                            current_x_coord * current_x_coord
                            + current_y_coord * current_y_coord
                        )
                        * coeff_1
                    )
                    * coeff_2
                )
                i += 1
    # unity kernel
    else:
        kernel[x_coord + y_coord * k_size] = 1.0

    # subtract second gaussian from kernel
    if sigma_2 > 0:
        coeff_1 = 1 / (2 * sigma_2 * sigma_2)
        coeff_2 = 1 / (2 * np.pi * sigma_2 * sigma_2)
        i = 0
        for current_y_coord in range(-y_coord, y_coord + 1):
            for current_x_coord in range(-x_coord, x_coord + 1):
                kernel[i] -= (
                    np.exp(
                        -(
                            current_x_coord * current_x_coord
                            + current_y_coord * current_y_coord
                        )
                        * coeff_1
                    )
                    * coeff_2
                )
                i += 1
    # unity kernel
    else:
        kernel[x_coord + y_coord * k_size] -= 1.0

    # zero-normalize scling kernel with scaling factor 1.0
    norm_kernel = normalize_kernel(kernel, k_size, k_size, scaling_factor=1.0)

    # apply filter with norm_kernel
    return cv2.filter2D(img, -1, norm_kernel.reshape(k_size, k_size))


def negate(img):
    """Negative of image"""

    return cv2.bitwise_not(img)


def get_black_white_indices(hist, tot_count, black_count, white_count):
    """Blacking and Whiting out indices same as color balance"""

    black_ind = 0
    white_ind = 255
    color = 0
    for i, value in enumerate(hist):
        color += value
        if color > black_count:
            black_ind = i
            break
    color = 0
    for i in range(len(hist) - 1, -1, -1):
        color += hist[i]
        if color > (tot_count - white_count):
            white_ind = i
            break

    return [black_ind, white_ind]


def contrast_stretch(img, black_point, white_point):
    """Contrast stretch image with black and white cap"""

    channel_hists = []
    # calculate histogram for each channel
    for channel in cv2.split(img):
        channel_hists.append(
            cv2.calcHist([channel], [0], None, [256], (0, 256)).flatten().tolist()
        )
    # get black and white percentage indices
    black_white_indices = []
    for hist in channel_hists:
        black_white_indices.append(
            get_black_white_indices(
                hist,
                img.shape[0] * img.shape[1],
                img.shape[0] * img.shape[1] * black_point / 100,
                img.shape[0] * img.shape[1] * white_point / 100,
            )
        )
    stretch_map = np.zeros((3, 256), dtype="uint8")

    # stretch histogram
    for curr_channel, value in enumerate(black_white_indices):
        black_ind, white_ind = value
        for i in range(stretch_map.shape[1]):
            if i < black_ind:
                stretch_map[curr_channel][i] = 0
            else:
                if i > white_ind:
                    stretch_map[curr_channel][i] = 255
                else:
                    if (white_ind - black_ind) > 0:
                        stretch_map[curr_channel][i] = (
                            round((i - black_ind) / (white_ind - black_ind)) * 255
                        )
                    else:
                        stretch_map[curr_channel][i] = 0
    # stretch image
    channel_stretch = []
    for i, channel in enumerate(cv2.split(img)):
        channel_stretch.append(cv2.LUT(channel, stretch_map[i]))

    return cv2.merge(channel_stretch)


def fast_gaussian_blur(img, ksize, sigma):
    """Gussian blur using linear separable property of Gaussian distribution"""

    kernel_1d = cv2.getGaussianKernel(ksize, sigma)
    return cv2.sepFilter2D(img, -1, kernel_1d, kernel_1d)


def gamma(img, gamma_value):
    """Gamma correction of image"""

    i_gamma = 1 / gamma_value
    lut = np.array(
        [((i / 255) ** i_gamma) * 255 for i in np.arange(0, 256)], dtype="uint8"
    )
    return cv2.LUT(img, lut)


def color_balance(img, low_cap, high_cap):
    """Contrast stretch image by histogram equilization with black and white cap"""

    tot_pix = img.shape[1] * img.shape[0]
    # no.of pixels to black-out and white-out
    low_count = tot_pix * low_cap / 100
    high_count = tot_pix * (100 - high_cap) / 100

    contrast_img = []
    # for each channel, apply contrast-stretch
    for channel in cv2.split(img):
        # cummulative histogram sum of channel
        cum_hist_sum = np.cumsum(cv2.calcHist([channel], [0], None, [256], (0, 256)))

        # find indices for blacking and whiting out pixels
        low_index, high_index = np.searchsorted(cum_hist_sum, (low_count, high_count))
        if low_index == high_index:
            contrast_img.append(channel)
            continue
        # lut with min-max normalization for [0-255] bins
        lut = np.array(
            [
                0
                if i < low_index
                else (
                    255
                    if i > high_index
                    else round((i - low_index) / (high_index - low_index) * 255)
                )
                for i in np.arange(0, 256)
            ],
            dtype="uint8",
        )
        # constrast-stretch channel
        cs_ch = cv2.LUT(channel, lut)
        contrast_img.append(cs_ch)

    return cv2.merge(contrast_img)


def whiteboard_enhance(img, destination):
    """Enhance Whiteboard image"""
    img = cv2.imread(img)
    contrast_black_per, contrast_white_per = 2, 99.5
    gauss_k_size, gauss_sigma = 3, 1
    gamma_value = 1.1
    color_balanced_black_per, color_balanced_white_per = 2, 1

    # Difference of Gaussian (DoG)
    # Negative of image
    negative_img = negate(dog(img, 15, 100, 0))
    # Contrast Stretch (CS)
    contrast_stretch_img = contrast_stretch(negative_img, contrast_black_per, contrast_white_per)
    # Gaussian Blur
    blur_img = fast_gaussian_blur(contrast_stretch_img, gauss_k_size, gauss_sigma)
    # Gamma Correction
    gamma_img = gamma(blur_img, gamma_value)
    # Color Balance (CB) (also Contrast Stretch)
    color_balanced_img = color_balance(
        gamma_img, color_balanced_black_per, color_balanced_white_per)

    cv2.imwrite(destination, color_balanced_img)
